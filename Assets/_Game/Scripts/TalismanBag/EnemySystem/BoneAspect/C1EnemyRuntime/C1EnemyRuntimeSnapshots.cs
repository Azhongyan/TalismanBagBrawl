...
namespace TalismanBag.EnemySystem.BoneAspect.C1EnemyRuntime
{
    public sealed class C1EnemyRuntimeProfileSnapshot
    {
        ...
        public C1EnemyRuntimeProfileSnapshot(
            ...
            string shellBreakCounterWindowKey,
            int requestedCounterWindowTicks,
            int deathPresentationHoldRecommendationTicks,
            bool devOnlyFixture,
            bool formalBalance,
            bool devOnly,
            bool isEnabled,
        ...
        public int DeathPresentationHoldRecommendationTicks { get; private set; }
        ...
        internal string CanonicalPayload()
        {
            return string.Join("|", new[]
            {
                ...
                C1EnemyCanonical.Number(DeathPresentationHoldRecommendationTicks),
    ...
    public sealed class C1EnemyActionPatternSnapshot
    {
        ...
        public C1EnemyActionPatternSnapshot(
            ...
            C1EnemyInterruptPolicy interruptPolicy,
            int priority,
            C1EnemyCueKind cueKind,
            string effectRequestLifetimeClass,
            ...
            bool devOnlyFixture)
        {
            ...
            Priority = priority;
            CueKind = cueKind;
            EffectRequestLifetimeClass = effectRequestLifetimeClass ?? string.Empty;
        ...
        public C1EnemyCueKind CueKind { get; private set; }
        public string EffectRequestLifetimeClass { get; private set; }
        ...
        internal string CanonicalPayload()
        {
            return string.Join("|", new[]
            {
                ...
                C1EnemyCanonical.Number(RequestedDurationTicks),
                C1EnemyCanonical.Flag(DevOnlyFixture)
            });
        }
    }
    ...
    public sealed class C1EnemyRuntimeSnapshot
    {
        ...
        public long NextActionDueTick
        {
            get
            {
                return _data.ActionScheduleStates.Count == 0
                    ? long.MaxValue
                    : _data.ActionScheduleStates.Min(value => value.NextDueTick);
            }
        }
        public IReadOnlyList<C1EnemyActionScheduleStateSnapshot> ActionScheduleStates
        {
            get { return C1EnemyCollection.Copy(_data.ActionScheduleStates); }
        }
    ...
    internal static class C1EnemyCanonical
    {
        public static string Full(C1EnemyRuntimeStateData data)
        {
            ...
            string schedules = string.Join(
                ",",
                data.ActionScheduleStates
                    .OrderBy(value => value.ActionPatternId, StringComparer.Ordinal)
                    .Select(Schedule));
            string requests = string.Join(",", data.PendingRequests.Select(Request));
            string cues = string.Join(",", data.Cues.Select(Cue));
            long nextDue = data.ActionScheduleStates.Count == 0
                ? long.MaxValue
                : data.ActionScheduleStates.Min(value => value.NextDueTick);
            return string.Join("|", new[]
            {
                data.SchemaId,
                ...
                string.Join(
                    ",",
                    data.AcceptedApplicationEventIds.OrderBy(
                        value => value,
                        StringComparer.Ordinal)),
                requests,
                cues,
                data.LatestCue == null ? string.Empty : Cue(data.LatestCue),
                string.Join(",", data.Errors.OrderBy(value => value, StringComparer.Ordinal)),
                string.Join(
                    ",",
                    data.PublicErrors.OrderBy(value => value, StringComparer.Ordinal)),
                string.Join(
                    ",",
                    data.DeveloperDiagnostics.OrderBy(value => value, StringComparer.Ordinal)),
        ...
        private static string Schedule(C1EnemyActionScheduleStateSnapshot schedule)
        {
            return string.Join("~", new[]
            {
                schedule.ActionPatternId,
                Number(schedule.NextDueTick),
                Number(schedule.ExecutionCount),
                Number(schedule.LastStartTick)
            });
        }

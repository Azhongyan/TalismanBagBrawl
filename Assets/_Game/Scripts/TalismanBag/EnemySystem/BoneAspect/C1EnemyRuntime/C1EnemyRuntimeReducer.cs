...
namespace TalismanBag.EnemySystem.BoneAspect.C1EnemyRuntime
{
    public static class C1EnemyRuntimeReducer
    {
            ...
            long battleTick)
        {
            ...
            if (battleTick < 0L)
            {
                throw new ArgumentOutOfRangeException("battleTick");
            }

            IReadOnlyList<C1EnemyActionPatternSnapshot> actions =
                C1EnemyRuntimeCatalog.FindActionsForProfile(runtimeProfileId);
            if (actions.Count == 0)
            {
                throw new InvalidOperationException("Runtime profile has no action pattern.");
            }
            ...
            C1EnemyBattleApplicationResult application)
        {
            ...
            if (application.ActualShellDeltaApplied > 0)
            {
                ...
                if (previousShell > 0 && data.CurrentShell == 0)
                {
                    ...
                    {
                        C1EnemyRuntimeProfileSnapshot profile =
                            C1EnemyRuntimeCatalog.FindProfile(data.RuntimeProfileId);
                        ...
                        AddRequest(
                            ...
                            application.ApplicationEventId,
                            string.Empty,
                            profile == null ? 0 : profile.RequestedCounterWindowTicks);
                        AddCue(
                            data,
            ...
            long battleTick)
        {
            ...
            if (profile == null || actions.Count == 0)
            {
                return Reject(snapshot, "PROFILE_OR_ACTION_MISSING");
            }
            C1EnemyRuntimeStateData data = NewData(
                profile,
                actions,
        ...
        private static C1EnemyRuntimeStateData NewData(
            C1EnemyRuntimeProfileSnapshot profile,
            IEnumerable<C1EnemyActionPatternSnapshot> actions,
            string enemyInstanceId,
            int resetGeneration,
            long battleTick)
        {
            List<C1EnemyActionScheduleStateSnapshot> schedules = actions
                .OrderBy(action => action.ActionPatternId, StringComparer.Ordinal)
                .Select(action => new C1EnemyActionScheduleStateSnapshot(
                    action.ActionPatternId,
                    checked(battleTick + action.FirstDueTick),
                    0,
                    -1L))
                .ToList();
            return new C1EnemyRuntimeStateData
            {
                ...
                ActionScheduleStates = schedules
            };
        }

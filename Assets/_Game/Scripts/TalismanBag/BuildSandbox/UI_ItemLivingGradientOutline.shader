Shader "TalismanBag/UI/ItemLivingGradientOutline"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            Name "ItemLivingGradientOutline"
            ...
            v2f vert(appdata_t v)
            {
                ...
                // The phase must be local to this Image, because a
                // SpriteAtlas UV rectangle is not centred at texture 0.5.
            ...
            {
                return saturate(
                    (tex2D(_MainTex, uv) + _TextureSampleAdd).a);
            }
            ...
            {
                float2 radius =
                    _MainTex_TexelSize.xy * _OutlineRadiusTexels;
                float2 diagonal = radius * 0.70710678;
                fixed4 sourceSample = tex2D(_MainTex, IN.texcoord);
                fixed centerAlpha = saturate(
                    (sourceSample + _TextureSampleAdd).a);

                // Exactly eight alpha-neighbour samples per shared pass.
                // Persistent core/halo overlap continuously; the trigger
                // pass is only visible during the 550 ms response.
                fixed n0 = SampleAlpha(IN.texcoord + float2(radius.x, 0));
                fixed n1 = SampleAlpha(IN.texcoord + float2(-radius.x, 0));
                fixed n2 = SampleAlpha(IN.texcoord + float2(0, radius.y));
                fixed n3 = SampleAlpha(IN.texcoord + float2(0, -radius.y));
                fixed n4 = SampleAlpha(IN.texcoord + diagonal);
                fixed n5 = SampleAlpha(IN.texcoord - diagonal);
                fixed n6 = SampleAlpha(
                    IN.texcoord + float2(diagonal.x, -diagonal.y));
                fixed n7 = SampleAlpha(
                    IN.texcoord + float2(-diagonal.x, diagonal.y));
                ...
                fixed neighbourReach =
                    saturate(maxNeighbour - centerAlpha);
                // Keep a continuous full-silhouette contour even where
                // bilinear alpha is faint at gameplay zoom. The moving
                // gold segment below is additive to this stable base.
                fixed contourCoverage = smoothstep(
                    0.018,
                    0.24,
                    neighbourReach);
                ...
                fixed innerEnergy =
                    saturate(centerAlpha - minNeighbour)
                    * _InnerEdgeStrength;
                fixed band =
                    saturate(outerBand + innerEnergy);
                ...
                float2 centered = IN.objectPosition;
                ...
                float directionalPhase =
                    dot(localDirection, float2(0.78, 0.36))
                    * 0.5 + 0.5;
                float fallbackNoise =
                    ProceduralNoise(floor(IN.texcoord * 48.0));
                float textureNoise =
                    tex2D(
                        _NoiseTexture,
                        IN.texcoord * 3.0
                ...
                float brightSegment = pow(
                    saturate(1.0 - segmentDistance), 3.2);
                float brightTail = pow(
                    saturate(1.0 - segmentDistance), 1.35)
                    * 0.34;
                ...
                fixed3 bodyEmission =
                    saturate(sourceSample.rgb) * bodyTint;
                bodyEmission +=
                    _HighlightColor.rgb
                    * saturate(_BodyWarmth)
                    * 0.12;

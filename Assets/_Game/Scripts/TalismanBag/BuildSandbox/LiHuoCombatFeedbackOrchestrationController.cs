...
namespace TalismanBag.BuildSandbox
{
        ...
        MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            IBattleSandboxAcceptedDamagePresentationSource source =
                ResolveAcceptedPresentationSource();
            int generation = source?.PresentationGeneration ?? 0;
            string sourceId = source?.PresentationSourceId
                ?? string.Empty;
            ...
            if (!enhancedMode
                || source?.HasActivePresentationSession != true
                || generation <= 0)
            {
                return;
            }
            ...
            IBattleSandboxAcceptedDamagePresentation accepted =
                source.LastAcceptedDamagePresentation;
            if (accepted == null
                || string.Equals(
                    accepted.EventId,
                    lastAcceptedEventId,
            ...
            IBattleSandboxAcceptedDamagePresentation accepted)
        {
            ...
            // The active Battle owner has already accepted and published
            // this read-only presentation event.
            ...
            Vector3 targetWorld)
        {
            ...
            int seed = StableHash(accepted.EventId);
            ...
            {
                activeChain = null;
                lastDiagnostic = "CHAIN_COMPLETE_SOURCE_ONLY";
            }
        }
        ...
        private RectTransform ResolveTargetAnchor(
            IBattleSandboxAcceptedDamagePresentation accepted)
        {
            if (accepted is C1OrdinaryEncounterApplicationPresentationEvent
                ordinary
                && ordinaryPresentationAdapter != null
                && ordinaryPresentationAdapter.TryResolveEnemyDamageAnchor(
                    ordinary.EnemySnapshot?.EnemyInstanceId,
                    out RectTransform ordinaryAnchor))
            {
                return ordinaryAnchor;
            }

            return visualCueAdapter?.DamageDealtAnchor;
        }
        ...
        private bool HasCorrelatedShellBreak(
            IBattleSandboxAcceptedDamagePresentation accepted)
        {
            if (accepted?.ShellBreakApplied == true)
            {
                return true;
            }
            BattleSandboxAcceptedItemPresentationEvent bossAccepted =
                accepted as BattleSandboxAcceptedItemPresentationEvent;
            IReadOnlyList<ShougunuPhase1BattleDisplayEvent> events =
                battleRuntime?.CurrentContext?.DisplayEvents;
            return bossAccepted != null
                && events != null
                && events.Any(value => value != null
                    && value.resetGeneration
                        == bossAccepted.resetGeneration
                    && value.battleTick == bossAccepted.battleTick
                    && string.Equals(
                        value.ledgerEventId,
                        bossAccepted.damageLedgerEventId,
                        StringComparison.Ordinal)
                    && value.channel
            ...
            ResolveAcceptedPresentationSource()
        {
            IBattleSandboxAcceptedDamagePresentationSource[] sources =
                Resources.FindObjectsOfTypeAll<MonoBehaviour>()
                    ...
                    .ToArray();
            acceptedPresentationSource = sources.Length == 1
                ? sources[0]
                : null;
            return acceptedPresentationSource;
        }
        ...
        private void CancelActiveChain(string reason)
        {
            ...
            StopAudioSourceIfAlive(sourceAudio);
            StopAudioSourceIfAlive(travelAudio);
            StopAudioSourceIfAlive(impactAudio);
            if (hadActive
                && !string.Equals(
                    reason,
                    "GENERATION_CHANGED",
        ...
        {
            cleanMode = active;
            TrySetShougunuDevControlsVisible(!active);
            Debug.Log(
        ...
        {
            TrySetShougunuDevControlsVisible(false);
            cleanMode = true;
        }

        private bool TrySetShougunuDevControlsVisible(bool visible)
        {
            ShougunuPhase1VisualPrototypeController owner = Resources
                .FindObjectsOfTypeAll<
                    ShougunuPhase1VisualPrototypeController>()
                .FirstOrDefault(value => value != null
                    && value.gameObject.scene == gameObject.scene);
            return owner != null
                && owner.SetDevControlsVisible(visible);
        }

        private static void StopAudioSourceIfAlive(AudioSource source)
        {
            if (source != null)
            {
                source.Stop();

...
namespace TalismanBag.BattleBridge.NianResource
{
    public sealed class BattleSandboxNianResourceEngine
    {
        ...
        public BattleSandboxNianPulseResult ApplyActualItemCostPulse(
            I031NianSourceSnapshot source,
            BattleSandboxNianActualItemCostPulseRequest request)
        {
            string preflightError = ValidateActualItemCostPulse(
                source,
                request,
                out I031NianCostRequestFact cost);
            if (!string.IsNullOrEmpty(preflightError))
            {
                return RejectActualWithoutMutation(
                    preflightError,
                    request,
                    cost);
            }

            List<BattleSandboxNianLedgerEntry> ledger =
                current.Ledger.ToList();
            List<string> processed =
                current.ProcessedPulseEventIds.ToList();
            int beforeGeneration = current.currentNian;
            int afterGeneration = Math.Min(
                BattleSandboxNianResourceSnapshot.MaxNian,
                beforeGeneration + source.generationPerPulse);
            ...
            ledger.Add(new BattleSandboxNianLedgerEntry(
                ...
                current.resetGeneration,
                BattleSandboxNianLedgerKind.Generation,
                request.itemRequestEventId,
                cost.requestFactId,
                cost.itemId,
                generated,
                beforeGeneration,
                afterGeneration,
                generated == source.generationPerPulse
                    ? "GENERATED"
                    : "GENERATED_CAPPED"));

            bool enough = afterGeneration >= cost.nianCost;
            int afterSpend = enough
                ? afterGeneration - cost.nianCost
                : afterGeneration;
            ...
            ledger.Add(new BattleSandboxNianLedgerEntry(
                ...
                afterSpend,
                reason));
            processed.Add(request.itemRequestEventId);
        ...
        private string ValidateActualItemCostPulse(
            I031NianSourceSnapshot source,
            BattleSandboxNianActualItemCostPulseRequest request,
            out I031NianCostRequestFact cost)
        {
            ...
            if (request == null)
            {
                return "ACTUAL_ITEM_PULSE_REQUEST_MISSING";
            }
            if (!string.Equals(
                    request.schemaId,
                    BattleSandboxNianActualItemCostPulseRequest.CurrentSchemaId,
                    StringComparison.Ordinal))
            {
                return "ACTUAL_ITEM_PULSE_SCHEMA_MISMATCH";
            }
            if (source.sourceGeneration != current.resetGeneration
                || request.expectedResetGeneration != current.resetGeneration)
            {
            ...
            if (!source.isOwned || source.location != I031Location.Board
                ...
                || !string.Equals(
                    ...
                    StringComparison.Ordinal)
                || !string.Equals(
                    source.specialIdentityId,
                    I031InventoryPlacementContract.SpecialIdentityId,
                    StringComparison.Ordinal)
                || !string.Equals(
                    source.stablePlacementId,
                    I031InventoryPlacementContract.StablePlacementId,
                    StringComparison.Ordinal))
            {
                return "SOURCE_PLACEMENT_SIGNATURE_INVALID";
            ...
            if (string.IsNullOrWhiteSpace(request.itemRequestEventId))
            {
                return "ITEM_REQUEST_EVENT_ID_MISSING";
            }
            if (current.ProcessedPulseEventIds.Contains(
                    request.itemRequestEventId,
                    StringComparer.Ordinal))
            {
                return "DUPLICATE_ITEM_REQUEST_EVENT";
            }
            if (request.battleTick !=
                (current.pulseCount + 1L) * PulseIntervalTicks)
            {
                return "PULSE_TICK_OUT_OF_SEQUENCE";
            }
            if (string.IsNullOrWhiteSpace(request.baseItemId))
            {
                return "BASE_ITEM_ID_MISSING";
            }

            cost = source.FindCostFact(request.baseItemId);
            if (cost == null)
            {
                return "ACTUAL_ITEM_COST_FACT_MISSING";
            }
            if (!string.Equals(
                    request.expectedCostRequestFactId,
                    cost.requestFactId,
        ...
        private BattleSandboxNianPulseResult RejectActualWithoutMutation(
            string reason,
            BattleSandboxNianActualItemCostPulseRequest request,
            I031NianCostRequestFact cost)
        {
            BattleSandboxNianResourceApplication application =
                new BattleSandboxNianResourceApplication(
                    BattleSandboxNianDecision.Rejected,
                    reason,
                    request?.itemRequestEventId ?? string.Empty,
                    cost?.requestFactId
                        ?? request?.expectedCostRequestFactId
                        ?? string.Empty,
                    cost?.itemId ?? request?.baseItemId ?? string.Empty,
                    cost?.nianCost ?? 0,
                    current.currentNian,
                    current.resetGeneration,
                    request?.battleTick ?? 0L);

...
namespace TalismanBag.BattleBridge.ShougunuPhase1
{
        ...
        IBattleSandboxAcceptedDamagePresentationSource
    {
        ...
        private IC1EncounterAdmissionAuthority encounterAdmissionAuthority;
        private MonoBehaviour encounterAdmissionOwner;
        ...
        public bool HasEncounterAdmissionAuthority =>
            ResolveEncounterAdmissionAuthority() != null;
        public bool BossEncounterAdmissionGranted =>
            IsBossEncounterAdmissionGranted();
        public C1EncounterAdmissionSnapshot EncounterAdmission =>
            ResolveEncounterAdmissionAuthority()?.EncounterAdmission;
        public string PresentationSourceId =>
            "SHOUGUNU_PHASE1_P3";
        public bool HasActivePresentationSession => HasActiveSession;
        public int PresentationGeneration => ResetGeneration;
        ...
        private void Update()
        {
            ...
            if (!IsBossEncounterAdmissionGranted())
            {
                RejectUnadmittedBattleSurface();
                return;
            }
        ...
        public void BindEncounterAdmissionAuthority(
            IC1EncounterAdmissionAuthority authority)
        {
            encounterAdmissionAuthority = authority;
            encounterAdmissionOwner = authority as MonoBehaviour;
        }
        ...
        private IC1EncounterAdmissionAuthority
            ResolveEncounterAdmissionAuthority()
        {
            if (encounterAdmissionAuthority != null
                && encounterAdmissionOwner != null
                && encounterAdmissionOwner.gameObject.scene
                    == gameObject.scene)
            {
                return encounterAdmissionAuthority;
            }

            IC1EncounterAdmissionAuthority[] authorities =
                Resources.FindObjectsOfTypeAll<MonoBehaviour>()
                    .Where(value => value != null
                        && value.gameObject.scene == gameObject.scene)
                    .OfType<IC1EncounterAdmissionAuthority>()
                    .ToArray();
            if (authorities.Length != 1)
            {
                encounterAdmissionAuthority = null;
                encounterAdmissionOwner = null;
                return null;
            }

            encounterAdmissionAuthority = authorities[0];
            encounterAdmissionOwner = authorities[0] as MonoBehaviour;
            return encounterAdmissionAuthority;
        }

        private bool IsBossEncounterAdmissionGranted()
        {
            IC1EncounterAdmissionAuthority authority =
                ResolveEncounterAdmissionAuthority();
            return authority != null
                && authority.IsShougunuPhase1StartAdmitted(
                    gameObject.scene.name);
        }

        private void RejectUnadmittedBattleSurface()
        {
            if (session != null
                || pendingStart
                || observedBattleMode
            ...
            lastDiagnosticDetail =
                "Generic BattleSandbox grid/button state cannot start "
                + "Shougunu P3. Admission requires ChapterFlow 1-10 "
                + "BossGate plus an accepted Manual Challenge request.";
        }

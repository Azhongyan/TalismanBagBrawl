...
5. 用户确认通过后，RepoOps 自动做小版本记录；该记录不等于授权 commit、tag、push 或回滚。
6. 用户确认通过且 RepoOps 记录完成后，开发窗口必须发送 `PASS_SYNC_BROADCAST` 给已注册固定角色，并取得 `PASS_SYNC_SENT`；没有真实发送能力时，必须生成手动转发同步包并停止等待。
7. `PASS_SYNC_BROADCAST` 只用于验收通过后的状态同步，不得用于新任务下发；新任务开始必须走 `SINGLE_LANE_TASK_INTAKE`。

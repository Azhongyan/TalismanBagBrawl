# 给 Claude Code 的多工具协作入场说明

日期：2026-06-25

接收方：Claude Code

本文用于让 Claude Code 快速理解《符箓背包》当前多工具协作方式。它不是授权文件，不替代 Codex 官方任务档案。

## 0. 先记住一句话

Codex 仍是本项目的主要开发工具、总控、最终验收与版本收口方。

Claude Code 未来可以作为“包体执行工具”参与开发，但只有在用户和 Codex 明确分配包体后才可以开工。

当前状态：

```text
Claude Code 尚未被正式分配包体。
Cross-System Executor 仍为 DISABLED。
本文件只用于入场理解与后续协同准备。
```
...
它的用途是“当前状态快照”，不是授权开发。你不需要每次读取 Codex、ZCode、Claude 的完整历史，只读 `00_SYNC_HUB.md` 中给你的摘要、状态卡和链接即可。
...
不要默认读取 ZCode 的完整日志。若确有依赖，只读 `00_SYNC_HUB.md` 指向的摘要或交接包。
...
| 包体 | 任务档案 | 风险 | 当前状态 | 是否可由 Claude 开工 |
| --- | --- | --- | --- | --- |
...
- 这两个包体当前都不是给 Claude 的授权工单，只是公告板事实。
...
```text
1. 在 00_SYNC_HUB.md 中确认包体状态和文件锁。
2. 建议使用独立 worktree：
   F:\Porject\TalismanBagBrawl_claude
3. 在 Docs/V0.3/MultiTool/WorktreeLocks/ 写你的锁文件。
4. 只读正式任务档案，不要自己改任务范围。
5. 先做本工具内部 Tech Architect 拆解。
6. 再写本工具内部 Task Writer 执行计划。
7. 再开发。
8. 再做本工具内部 QA。
9. 完工后写 Receipts / QA / Handoffs。
10. 更新 00_SYNC_HUB.md 中 Claude Code 状态卡和交接链接。
11. 等 Codex 最终收口。
```

## 5. Claude Code 内部也要模拟 Codex 的回转流程

你可以在 Claude Code 内部模拟以下角色，但这些只是你自己的工作步骤，不是项目官方固定角色：
...
```text
Docs/V0.3/MultiTool/Receipts/<包体名>_LOCAL_PLAN_FROM_CLAUDE.md
```
...
```text
Docs/V0.3/MultiTool/QA/<包体名>_QA_REPORT_FROM_CLAUDE.md
```
...
```text
Docs/V0.3/MultiTool/Receipts/<包体名>_DEV_RECEIPT_FROM_CLAUDE.md
Docs/V0.3/MultiTool/QA/<包体名>_QA_REPORT_FROM_CLAUDE.md
Docs/V0.3/MultiTool/Handoffs/<包体名>_HANDOFF_SUMMARY_FROM_CLAUDE.md
```
...
2. 写清失败原因和复现路径。
3. 不要直接扩范围修。
4. 更新 00_SYNC_HUB.md 状态为 RETURNED 或 BLOCKED。
5. 交回 Codex，由 Codex 决定是否进入 Tech Architect 失败归因和 Task Writer 修复工单。
```

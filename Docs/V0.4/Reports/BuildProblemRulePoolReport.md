# Build Problem Rule Pool Report
...
- Rule schema layer only for V0.4 BuildSandbox Phase 2.
- No formal chapter, combat connection, visible page, scene edit, product data edit, reward edit, drop edit, Boss edit, numeric edit, or persistence edit.
- All new config types default to `devOnly=true` and `isEnabled=false`.
- FeatureFlags remain default false.

## Added Config Types

| Type | Purpose | Default Isolation |
| --- | --- | --- |
| `MapRuleConfig` | Declares how a map rule asks a Build problem. | `devOnly=true`, `isEnabled=false` |
| `EnemyProblemConfig` | Declares how an enemy exposes a Build gap. | `devOnly=true`, `isEnabled=false` |
| `BossProblemConfig` | Declares a multi-key Boss Build lock. | `devOnly=true`, `isEnabled=false` |
| `BossProblemKeyRequirement` | Serializable key requirement inside Boss problem configs. | `devOnly=true`, `isEnabled=false` |
| `BuildReadinessCheckConfig` | Reports readiness before a problem is attempted. | `devOnly=true`, `isEnabled=false`, `reportsOnly=true` |
| `WeaknessWindowConfig` | Describes a devOnly weakness window for reports and later preview. | `devOnly=true`, `isEnabled=false` |
| `DropBiasConfig` | Describes devOnly drop tendency for future seed data. | `devOnly=true`, `isEnabled=false`, `reportsOnly=true` |
| `FailureHintConfig` | Describes report-only failure hints. | `devOnly=true`, `isEnabled=false`, `reportsOnly=true` |
...
- Formal 1-10 / 2-10 flow: not referenced by this package.
- Product enemy, Boss, reward, drop, upgrade, numeric, progress, and persistence data: not edited.
- Current UI scenes and hand-tuned layout: not edited.
- BuildReadiness output is report-only and does not write product panels.
- DropBias output is report-only and does not touch product drop tables.

## Next SeedData Gate

- `V0.4-BuildProblemSeedData01` should add `10` map rule seeds.
- `V0.4-BuildProblemSeedData01` should add `10` enemy problem seeds.
- `V0.4-BuildProblemSeedData01` should add `6` Boss problem seeds.
- Seed data must remain devOnly and disabled until a later Guard assignment changes the boundary.

# Build Problem Seed Data Report

Package: `V0.4-BuildProblemSeedData01`
Generated: `2026-07-01 10:19:33`
Status: `PASS`
Errors: `0`
Warnings: `0`
...
- Static devOnly seed provider for V0.4 BuildSandbox Phase 2 problem data.
- No product chapter, combat, UI scene, persistence, Boss list, reward, drop, numeric, V02, or V03 data connection.
- Every seed remains `devOnly=true` and `isEnabled=false`.
...
| Boss | Goal | Keys | Weakness Windows | DropBias | Problem Attributes |
| --- | --- | ---: | ---: | ---: | --- |
| 黑炉护壳师 | 验证惊雷破盾 Build | 4 | 1 | 3 | `BreakPower;BurstWindow` |
| 秽签梦母 | 验证净厄反制 Build | 4 | 1 | 3 | `CleansePower;ControlPower` |
| 偷灵炉心 | 验证聚能 / 镇魂 / 供能稳定 Build | 4 | 1 | 3 | `EnergyStability;ControlPower;BurstWindow` |
| 叩阵铜将 | 验证护阵防守 Build | 4 | 1 | 3 | `GuardPower;PlacementShape;EnergyStability` |
| 纸人千面阵 | 验证离火 / 清场 / 连锁 Build | 4 | 1 | 3 | `ClearPower;BurstWindow;ControlPower` |
| 黑炉复合阵眼 | 验证综合 Build 毕业考试 | 5 | 1 | 3 | `BreakPower;CleansePower;ControlPower;GuardPower;EnergyStability;ClearPower;BurstWindow` |

## Required Problem Attributes

`BreakPower`, `CleansePower`, `ControlPower`, `GuardPower`, `EnergyStability`, `ClearPower`, and `BurstWindow` are covered by Boss attributes and key requirements.

## Formal Flow Leak Check

- Formal 1-10 / 2-10: not referenced by this package.
...
- DropBias remains report-only and does not touch product drops.
- BossProblem seeds do not reference product Boss lists.

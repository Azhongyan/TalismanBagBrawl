...
Status: `BLOCKED_VERTICAL_SLICE_ERRORS`
Errors: `14`
Warnings: `0`
Leak Count: `1`
...
- Visual surface stays in the V04 preview scene and uses the existing battle-like preview area as the V0.2/V0.3 BattlePrepare-facing shell.
- Interaction authority is V0.4 ShapePlacement: `ShapePlacementSession`, `ShapeAwareItemTrayGrid`, and `MobileShapePlacementInputExtension`.
- The preview tray exposes the primary x2 item plus additional test items; click opens item info only, the item-info popup Rotate changes tray orientation, and drag starts placement.
- `Vertical2` is rendered in the tray as a real two-cell span: vertical by default, horizontal after popup Rotate, and back to vertical on the next popup Rotate.
- The tray reserves the secondary occupied slot visually, hides its empty card, and draws the spanning item card above slot backgrounds.
- Popup Rotate is the only rotation path; dragging, previewing, and board placement cannot rotate.
- Valid release commits immediately; invalid release returns the item to tray. There is no Ghost click confirm requirement.
- No formal V02/V03 scene, RunFlow, save, Boss, reward, drop, or numeric system is connected by this package.
...
| Battle-like preview area | `True` |
| Board grid present | `True` |
| Item tray present | `True` |
| Rotate button present | `True` |
| Cancel button present | `True` |
| Build Settings contains preview scene | `False` |
| Board slots | `25` |
| Tray slots | `40` |
| Categories | `6` |
...
| --- | --- |
| Item count | `23` |
| Item id | `preview_stone_core` |
| Shape id | `Square4` |
| Shape cell count | `4` |
| Tray x2 display | `False` |
| Tray x2 visual cell span | `False` |
| Tray vertical default | `False` |
| Tray horizontal after Rotate | `False` |
| Popup Rotate back to vertical | `False` |
| Tray invalid Rotate rejected | `False` |
| Tray occupied cells | `0,0;1,0;0,1;1,1` |
| Tray occupied slots | `0;1;5;6` |
...
| Item card click does not rotate | `True` |
| Drag starts placement | `True` |
| Rotate uses item info popup Rotate button | `True` |

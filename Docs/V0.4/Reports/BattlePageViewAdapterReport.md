# Battle Page View Adapter Report

Package: `V0.4-BattlePageViewAdapter01`
Generated: `2026-07-01 12:05:52`
Status: `PASS`
Errors: `0`
Warnings: `0`
...
- Creates a read-only visual language adapter for the V04 BattleSandboxPreview scene.
- Reuses battle prepare specifications, naming, and interaction language without reusing formal GameObjects.
- Does not write V02/V03 scenes, formal UI, RectTransform values, formal save data, or formal battle flow.
...
- FeatureFlags all disabled: `True`

## Output Specs

- Output spec count: `10`
- Reference mode: `ReadOnlySpecNoFormalGameObjectReuse`
- Reuses formal GameObjects: `False`
- Writes formal UI: `False`

| Section | Preview Slot | Formal Reference | Reference Only | Writes Formal UI | Contract |
| --- | --- | --- | --- | --- | --- |
| `BoardGrid` | `BoardGridPreview` | `V02FormationGridFrame` | `True` | `False` | Use battle-board scale and placement affordance as a V04 preview slot only. |
| `ItemTray` | `ItemTrayPreview` | `V03BattlePrepareItemTrayRoot` | `True` | `False` | Use the battle item tray as a V04 grid inventory preview language. 5x8, visibleRows=5, scroll=True. |
| `CategoryTabs` | `ItemTrayCategoryTabs` | `ItemTrayCategoryTabs` | `True` | `False` | Category filter tabs: All/Talisman/Tool/Material/Consumable/Special. |
| `SelectedItemInfo` | `SelectedItemInfo` | `Item tooltip / selected item info language` | `True` | `False` | Show selected item name, tags, shape, affix summary, and upgrade/placement hints in V04 only. |
| `PlacementFeedback` | `PlacementFeedback` | `Battle prepare placement feedback language` | `True` | `False` | Report placement legality and Build readiness hints without moving formal board objects. states=CanPlace/OutOfBounds/Overlap/MissingEnergy/ShapeRotated/SynergyChanged. |
| `ActionButtons` | `DevOnlyControlBar` | `V03BattleStateButton / V03PrepareToggleButton` | `True` | `False` | Action affordances: EnterPrepare/ContinueBattle/RunSimulation/ResetPreview/ExportReport. |
| `DataPanelDock` | `BuildSandboxDataPanelDock` | `None` | `True` | `False` | Dock binds V04 data panels only: BuildSummaryPanelSlot/SynergyPanelSlot/ShapeOccupancyPanelSlot/AffixModifierPanelSlot/ProblemReadinessPanelSlot/SimulationResultPanelSlot. |
| `BattleUiReuse` | `UnifiedBattleFeedbackLanguage` | `Existing battle feedback language` | `True` | `False` | Reuse channels=5, dedicatedPanels=False. |
| `DeveloperTuningPanel` | `BuildSandboxDataPanelDock` | `None` | `True` | `False` | Developer fields=8, playerChineseOnly=True, englishKeysPlayerVisible=False. |
| `PlayerHintMasking` | `PlayerHintPreview` | `None` | `True` | `False` | Masking rules=6, playerChineseOnly=True, fullAnswersPlayerVisible=False. |

## Battle Prepare Visual Language

- BoardGrid: `800x800` via `BoardGridPreview`.
- ItemTray: `800x800`, columns=`5`, rows=`8`, visibleRows=`5`.
- ItemTray slots: `40`, slotSize=`104x104`, spacing=`14x14`.
- Categories: `全部 / 符箓 / 法器 / 材料 / 消耗 / 特殊`.
- DataPanelDock slots: `BuildSummaryPanelSlot / SynergyPanelSlot / ShapeOccupancyPanelSlot / AffixModifierPanelSlot / ProblemReadinessPanelSlot / SimulationResultPanelSlot`.

## UI Reuse And Masking

- Battle UI reuse channels: `5`.
- Developer tuning fields: `8`.
- Player hint masking rules: `6`.
- Player UI Chinese-only: `True`.
- Player UI shows English stable keys: `False`.
- English stable keys limited to config/report/developer data-panel fields: `True`.
- Player Hint Preview shows clues, combat feedback, atmosphere hints, and failure feedback only.
- Developer Tuning View may show full rules, keys, thresholds, DropBias, and readiness data.

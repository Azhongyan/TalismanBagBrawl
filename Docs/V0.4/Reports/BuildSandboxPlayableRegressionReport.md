# BuildSandbox Playable Regression Report

Package: `V0.4-BuildSandboxPlayableRegression01`
Generated: `2026-07-04 01:43:43`
Status: `PASS`
Errors: `0`
Warnings: `0`
...
- Final regression for the V0.4 BuildSandbox playable preview only.
- Confirms the sandbox is ready for one full user handtest pass.
- Does not promote 3-10 / 4-10 into formal content.
- Does not open feature flags, write formal saves, write formal rewards/drops, or run formal chapter progress.
- Does not run scene binders or move current hand-tuned UI layout.

## Regression Summary

| Area | Result | Evidence |
| --- | --- | --- |
| V04 scene open/play readiness | `PASS` | sceneExists=True; binding=True |
| x2/x3/x4 tray coverage | `PASS` | x2=3; x3=2; x4=2 |
| Rotate scope | `PASS` | tray info rotate path guarded |
| Drag/place/illegal return | `PASS` | legal=2; illegalReturn=2 |
| Continue battle board read | `PASS` | combat preview reads current board snapshot |
| Build-dependent Boss feedback | `PASS` | bossState=9; cast=8; mechanic=28; logs=81; distinctBuildFeedback=4 |
| 3-10 / 4-10 devOnly balance rows | `PASS` | 3-10=2; 4-10=2 |
| Player answer leak check | `PASS` | leaks=0 |
| Feature flags default false | `PASS` | defaultTrue=0 |
| devOnly/isEnabled isolation | `PASS` | devOnlyFalse=0; isEnabledTrue=0 |
| Formal scope isolation | `PASS` | formalLeaks=0 |

## User Handtest Checklist

1. Open `Scene_TalismanBag_V04_BattleSandboxPreview` directly, press Play, and confirm Console has no red Error or yellow Warning from this package.
2. Confirm the item tray visibly contains x2, x3, and x4 multi-cell items.
3. Select tray items, use the item info rotate control, and confirm dragging or board-placed items do not rotate unexpectedly.
4. Drag x2/x3/x4 items to legal board cells and confirm they remain placed without snapping back, jumping, or disappearing.
5. Drag an item out of bounds or over occupied cells and confirm it returns cleanly to the tray/state before the drag.
6. Use prepare to continue battle after changing the board, then confirm Boss status, cast bar, floating mechanic text, and combat log refresh.
7. Try at least two different build layouts and confirm Boss feedback changes in visible wording or timing.
8. Review 3-10 / 4-10 devOnly balance rows and confirm the difficulty suggestions are readable.
9. Confirm player-visible UI does not show complete solution keys, answer weights, required tags, required affixes, required stats, or full Boss key answers.
10. Confirm no formal run flow, save data, rewards, drops, chapter progress, or feature flag activation appears during the sandbox test.

## Checklist Rows

| ID | Area | Result | User Handtest | Check | Evidence |
| --- | --- | --- | --- | --- | --- |
| `REG-01` | Scene | `PASS` | `True` | Open V04 sandbox scene from a cold editor state. | sceneExists=True; binding=True |
| `REG-02` | Tray | `PASS` | `True` | x2 / x3 / x4 items are present in the item tray. | x2=3; x3=2; x4=2 |
| `REG-03` | Rotation | `PASS` | `True` | Rotation is only triggered from selected tray item info controls. | RotateSelectedItem/RotateTrayItem/TryCommitTrayRotation found. |
| `REG-04` | Placement | `PASS` | `True` | Legal placement succeeds and illegal placement returns cleanly. | legal=2; illegalReturn=2 |
| `REG-05` | Continue Battle | `PASS` | `True` | Prepare to continue battle reads the current board and refreshes Boss feedback. | bossReadinessRows=9; placedSnapshotRead=True |
| `REG-06` | Build Feedback | `PASS` | `True` | Different builds produce different Boss state, cast, floating text, and combat log lines. | bossState=9; cast=8; mechanic=28; logs=81; distinctBuildFeedback=4 |
| `REG-07` | Dev Balance | `PASS` | `False` | 3-10 and 4-10 devOnly balance recommendations are generated and readable. | 3-10=2; 4-10=2 |
| `REG-08` | Player Leak | `PASS` | `True` | Player UI does not leak complete answer fields or forbidden solution keys. | playerLeaks=0 |
| `REG-09` | Feature Flags | `PASS` | `False` | All BuildSandbox feature flags remain default false. | defaultTrue=0 |
| `REG-10` | devOnly | `PASS` | `False` | All checked sandbox surfaces stay devOnly=true and isEnabled=false. | devOnlyFalse=0; isEnabledTrue=0 |
| `REG-11` | Formal Scope | `PASS` | `False` | No formal run flow, save, reward, drop, chapter, or settings write is present. | formalLeaks=0 |
| `REG-12` | Scene/Layout Scope | `PASS` | `False` | No formal scene or current V04 hand-tuned layout write is required by regression. | formalSceneTouched=false; currentLayoutWrite=false |
| `REG-13` | Console | `PASS` | `True` | Regression validator has zero Error and zero Warning issues. | errors=0; warnings=0 |
...
| BuildGrid Interaction ItemInfoPanel Samples | `PASS` | 0 | 0 | 10 |
...
| BuildSandbox Playable Regression 01 | `PASS` | 0 | 0 | 11 |
...
| `Info` | `PLAYABLE_REGRESSION_SCENE_READY` | V04 sandbox scene can be opened and has required battle sandbox bindings. | `V0.4-BuildSandboxPlayableRegression01` |
| `Info` | `PLAYABLE_REGRESSION_SHAPES_READY` | Tray contains x2/x3/x4 multi-cell item coverage. | `V0.4-BuildSandboxPlayableRegression01` |
| `Info` | `PLAYABLE_REGRESSION_ROTATION_SCOPE_READY` | Rotation remains scoped to selected tray item info controls. | `V0.4-BuildSandboxPlayableRegression01` |
| `Info` | `PLAYABLE_REGRESSION_PLACEMENT_READY` | Legal placement and illegal-return samples are covered. | `V0.4-BuildSandboxPlayableRegression01` |
| `Info` | `PLAYABLE_REGRESSION_BOARD_READ_READY` | Build combat preview reads the current board snapshot. | `V0.4-BuildSandboxPlayableRegression01` |
| `Info` | `PLAYABLE_REGRESSION_FEEDBACK_READY` | Different build layouts produce boss state, cast, mechanic, and combat log feedback. | `V0.4-BuildSandboxPlayableRegression01` |
| `Info` | `PLAYABLE_REGRESSION_DEV_BALANCE_READY` | 3-10 and 4-10 devOnly balance run rows are present and readable. | `V0.4-BuildSandboxPlayableRegression01` |
| `Info` | `PLAYABLE_REGRESSION_PLAYER_LEAK_CLEAR` | Player-side forbidden answer leak counters are zero. | `V0.4-BuildSandboxPlayableRegression01` |
| `Info` | `PLAYABLE_REGRESSION_FLAGS_FALSE` | All BuildSandbox feature flags still default false. | `V0.4-BuildSandboxPlayableRegression01` |
| `Info` | `PLAYABLE_REGRESSION_DEVONLY_DISABLED` | All checked sandbox surfaces stay devOnly=true and isEnabled=false. | `V0.4-BuildSandboxPlayableRegression01` |
| `Info` | `PLAYABLE_REGRESSION_FORMAL_SCOPE_CLEAR` | No formal flow, save, reward, drop, chapter, scene, or layout-write scope is used. | `V0.4-BuildSandboxPlayableRegression01` |

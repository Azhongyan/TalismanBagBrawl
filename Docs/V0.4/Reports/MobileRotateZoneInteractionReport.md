...
- `2026-07-08` white-key transparent material helper: added `UI_WhiteKeyTransparent.mat` / `UI_WhiteKeyTransparent.shader` for UI Image sequence frames exported from video with a white background. The material keys out pixels near `Key Color` with adjustable `White Tolerance` and `Edge Softness`, preserving normal UI stencil/clip behavior and leaving the source frame files unchanged.
...
- `2026-07-08` prepare/continue sprite sequence helper: added `BuildSandboxSpriteSequencePlayer` and a single `Battle Prepare Transition Sequence Player` slot on `BuildGridInteractionPreviewController`. Opening prepare mode and continuing battle both trigger the same optional sprite sequence player, so one authored prop can play the same frame animation for both transitions.
- `2026-07-08` prepare/continue sequence binding fix: if the controller slot is left empty, `BuildGridInteractionPreviewController` now resolves a `BuildSandboxSpriteSequencePlayer` at runtime from its children, then from the scene with a preferred object name of `guajian`, and finally from the first player with frames. This preserves manual Inspector binding when present while allowing the authored `guajian` sequence to play without editing scene RectTransforms.
- `2026-07-08` editable item-info panel helper: `BuildSandboxItemInfoPanel_Runtime` is now created/bound as a normal Edit Mode hierarchy object by `BuildGridInteractionPreviewController.OnValidate`, mirroring the placement-feedback authoring path. Runtime code reuses the scene object and no longer marks existing scene-backed detail panels as `DontSave`; generated default layout is applied only to newly created panel pieces so hand-tuned `PanelRoot` position and size are preserved.

...
| --- | --- | --- | --- | --- |
| `readinessCheckId` | `string` | `readiness_check_schema` | Unique readiness check id. | devOnly seed id only. |
| `checkType` | `string` | `tag_threshold` | Readiness rule type. | Report-only. |
| `requiredBuildTags` | `List<string>` | `empty` | Tags required to solve the problem. | Schema only. |
| `requiredSynergyIds` | `List<string>` | `empty` | Synergies required to solve the problem. | Schema only. |
| `requiredScore` | `int` | `1` | Pass score. | Report-only. |
| `failureHintIds` | `List<string>` | `empty` | Hints used when readiness fails. | No product UI write. |
| `reportsOnly` | `bool` | `true` | Prevents runtime product panel writes. | Must stay true. |

## WeaknessWindowConfig
| Field | Type | Default | Purpose | Isolation |
| --- | --- | --- | --- | --- |
| `weaknessWindowId` | `string` | `weakness_window_schema` | Unique weakness window id. | devOnly seed id only. |
| `windowType` | `string` | `timed_vulnerability` | Window category. | Preview-only. |
| `startSecond` | `float` | `0` | Preview start time. | No product combat timing write. |
| `durationSecond` | `float` | `3` | Preview window duration. | No product combat timing write. |
| `exposedBuildTags` | `List<string>` | `empty` | Tags exposed by the weakness. | Schema only. |
| `recommendedSynergyIds` | `List<string>` | `empty` | Synergies recommended in the window. | Schema only. |

## FailureHintConfig
| Field | Type | Default | Purpose | Isolation |
| --- | --- | --- | --- | --- |
| `failureHintId` | `string` | `failure_hint_schema` | Unique failure hint id. | devOnly seed id only. |
| `hintType` | `string` | `missing_build_key` | Hint category. | Report-only. |
| `priority` | `int` | `1` | Hint ordering. | Report-only. |
| `headline` | `string` | `Build readiness hint` | Short report heading. | No product UI write. |
| `detail` | `string` | `devOnly failure hint schema.` | Long report detail. | No product UI write. |
| `recommendedBuildTags` | `List<string>` | `empty` | Recommended tags. | Schema only. |
| `reportsOnly` | `bool` | `true` | Prevents runtime product UI writes. | Must stay true. |

## Isolation

- Readiness checks default to report-only.
- Weakness windows default to simulator-readable preview data only.
- Failure hints default to report-only copy and do not write product UI.

# Mechanic Hint Feedback Preview Report
...
- Adds a BuildSandbox/devOnly preview for map mechanics, enemy mechanics, boss skills, weakness windows, drop-bias atmosphere, and failure feedback.
- Reuses mature battle feedback channels: `BattleHint`, `DamageFeedback`, `CombatLog`, `Tooltip`, and `BossInfo`.
- Player-side rows contain Chinese hint/feedback/atmosphere copy only.
- Full solution fields remain linked to the developer data panel and are masked from player UI.
- Does not write formal RunFlow, SaveData, Boss, reward, drop, numeric config, V02/V03 scenes, or hand-tuned RectTransforms.

## Preview Summary

- Reference mode: `PlayerHintAdapterNoFormalUiWrite`
- Source preview build id: `buildsandbox_preview_context`
- Player visible rows: `67`
- Reuse channels: `5`
- Developer data panel links: `6`
- Feedback extension rows: `67`
...
| Key | Category | Reuse Channel | Chinese Display | Player Text | Atmosphere |
| --- | --- | --- | --- | --- | --- |
| `mapMechanic.dev_map_black_furnace_smoke` | `mapMechanic` | `BattleHint` | 地图机制线索 | 【黑炉烟尘】压上阵面时，场上会偏向破盾、清场。先看棋盘气息和供能变化，再决定是否换位。 | 灯火、地气和阵眼变化只做氛围提示，不显示解法清单。 |
| `mapMechanic.dev_map_bluestone_damp` | `mapMechanic` | `BattleHint` | 地图机制线索 | 【青石回潮】压上阵面时，场上会偏向净厄、供能。先看棋盘气息和供能变化，再决定是否换位。 | 灯火、地气和阵眼变化只做氛围提示，不显示解法清单。 |
| `mapMechanic.dev_map_lamp_shortage` | `mapMechanic` | `BattleHint` | 地图机制线索 | 【灯火不足】压上阵面时，场上会偏向短窗爆发、镇魂、清场。先看棋盘气息和供能变化，再决定是否换位。 | 灯火、地气和阵眼变化只做氛围提示，不显示解法清单。 |
| `mapMechanic.dev_map_copper_bell_night` | `mapMechanic` | `BattleHint` | 地图机制线索 | 【铜铃夜响】压上阵面时，场上会偏向镇魂、护阵。先看棋盘气息和供能变化，再决定是否换位。 | 灯火、地气和阵眼变化只做氛围提示，不显示解法清单。 |
| `mapMechanic.dev_map_formation_eye_shift` | `mapMechanic` | `BattleHint` | 地图机制线索 | 【阵眼偏移】压上阵面时，场上会偏向摆放形态、供能、护阵。先看棋盘气息和供能变化，再决定是否换位。 | 灯火、地气和阵眼变化只做氛围提示，不显示解法清单。 |
| `mapMechanic.dev_map_talisman_paper_damp` | `mapMechanic` | `BattleHint` | 地图机制线索 | 【符纸受潮】压上阵面时，场上会偏向净厄、供能、短窗爆发。先看棋盘气息和供能变化，再决定是否换位。 | 灯火、地气和阵眼变化只做氛围提示，不显示解法清单。 |
| `mapMechanic.dev_map_old_lane_echo` | `mapMechanic` | `BattleHint` | 地图机制线索 | 【旧巷回音】压上阵面时，场上会偏向清场、短窗爆发、连锁清场。先看棋盘气息和供能变化，再决定是否换位。 | 灯火、地气和阵眼变化只做氛围提示，不显示解法清单。 |
| `mapMechanic.dev_map_furnace_ash_fall` | `mapMechanic` | `BattleHint` | 地图机制线索 | 【炉灰落阵】压上阵面时，场上会偏向净厄、护阵。先看棋盘气息和供能变化，再决定是否换位。 | 灯火、地气和阵眼变化只做氛围提示，不显示解法清单。 |
| `mapMechanic.dev_map_night_watch_lamp_out` | `mapMechanic` | `BattleHint` | 地图机制线索 | 【夜巡灯灭】压上阵面时，场上会偏向短窗爆发、镇魂、破盾。先看棋盘气息和供能变化，再决定是否换位。 | 灯火、地气和阵眼变化只做氛围提示，不显示解法清单。 |
| `mapMechanic.dev_map_bluestone_crack` | `mapMechanic` | `BattleHint` | 地图机制线索 | 【青石裂纹】压上阵面时，场上会偏向摆放形态、供能、短窗爆发。先看棋盘气息和供能变化，再决定是否换位。 | 灯火、地气和阵眼变化只做氛围提示，不显示解法清单。 |
| `enemyMechanic.dev_enemy_shield_problem` | `enemyMechanic` | `CombatLog` | 敌人机制线索 | 【护盾】会制造护盾压力。补惊雷破盾、破甲词条或短窗爆发。 | 敌人压力通过战斗日志和状态提示露出，不展示硬解标签。 |
| `enemyMechanic.dev_enemy_swarm_problem` | `enemyMechanic` | `CombatLog` | 敌人机制线索 | 【群怪】会制造群怪压力。补离火清场、连锁或短冷却词条。 | 敌人压力通过战斗日志和状态提示露出，不展示硬解标签。 |

## Developer Data Panel Links

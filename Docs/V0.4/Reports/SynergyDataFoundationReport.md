# Synergy Data Foundation Report

Package: `V0.3/V0.4-BuildSandbox-SynergyDataFoundation01`
...
| Type | Count | Purpose |
| --- | ---: | --- |
| `ItemTagConfig` | 4 | Item tags, category, energy cost, and rarity gates. |
| `SynergyConfig` | 3 | Synergy ids, required tags, thresholds, and preview conditions. |
| `SynergyThresholdConfig` | 9 | Supports 2 / 4 / 6 / 8 piece tiers. |
| `PlacementConditionConfig` | 5 | Data-only placement requirements. |
| `EnergyConditionConfig` | 5 | Data-only energy requirements. |
| `BuildArchetypeConfig` | 2 | Build archetype seeds for later simulator packages. |
...
## Synergy Threshold Support

| Synergy | Thresholds |
| --- | --- |
| `bs_synergy_corruption_ward` | `2, 4` |
| `bs_synergy_guardian_energy` | `2, 4, 6` |
| `bs_synergy_lihuo_talisman` | `2, 4, 6, 8` |
...
- devOnly / isEnabled: every BuildSandbox config must remain `devOnly=true` and `isEnabled=false`.
- FeatureFlag default true: none when this report is PASS.
- Formal combat, drops, forge, UI, save, reward, boss, and numeric systems: not connected by this package.
- SynergyEvaluator and item shape occupancy logic: not implemented in this package.

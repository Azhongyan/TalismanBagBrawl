# Player Hint Masking Spec Report
...
- English stable keys are limited to config/report/developer data-panel field layers: `True`
- Developer tuning panel may show full rules: `True`
- Player Hint Preview shows full answers: `False`

## Masking Rules

| English Stable Key | Chinese Display Name | Player Visible | Developer Data Panel | Config | Report | Player Chinese Hint Policy |
| --- | --- | --- | --- | --- | --- | --- |
| `hardSolutionTags` | 硬解标签 | `False` | `True` | `True` | `True` | 显示模糊能力方向和失败反馈，不显示完整标签列表。 |
| `requiredSynergy` | 必需协同 | `False` | `True` | `True` | `True` | 显示协同倾向线索，不显示必须选择的完整协同答案。 |
| `requiredAffix` | 必需词缀 | `False` | `True` | `True` | `True` | 显示词缀方向提示，不显示必须刷到的完整词缀答案。 |
| `requiredStats` | 必需属性 | `False` | `True` | `True` | `True` | 显示压力类型和短板反馈，不显示达标阈值完整答案。 |
| `dropBiasWeights` | 定向倾向权重 | `False` | `True` | `True` | `True` | 玩家不显示权重；仅可显示掉落倾向的中文氛围线索。 |
| `bossSixKeyFullAnswer` | Boss六钥匙完整答案 | `False` | `True` | `True` | `True` | 玩家只看阶段线索和失败反馈；完整钥匙组合仅在开发者数据面板。 |

## Developer Field Layer

| English Stable Key | Chinese Display Name | V04 Data Panel Slot | Developer Only |
| --- | --- | --- | --- |
| `buildSummary` | 构筑概览 | `BuildSummaryPanelSlot` | `True` |
| `synergySummary` | 协同概览 | `SynergyPanelSlot` | `True` |
| `shapeOccupancy` | 格位占用 | `ShapeOccupancyPanelSlot` | `True` |
| `affixModifier` | 词缀修正 | `AffixModifierPanelSlot` | `True` |
| `problemReadiness` | 题目准备度 | `ProblemReadinessPanelSlot` | `True` |
| `simulationResult` | 模拟结果 | `SimulationResultPanelSlot` | `True` |
| `dropBiasPreview` | 定向倾向 | `ProblemReadinessPanelSlot` | `True` |
| `bossSixKeyFullAnswer` | Boss六钥匙完整答案 | `ProblemReadinessPanelSlot` | `True` |

## Player-Side Proof

- hardSolutionTags, requiredSynergy, requiredAffix, requiredStats, DropBias weights, and Boss six-key full answer are masked from player UI.
- Player-side copy is Chinese-only clue/feedback text; it does not include English stable keys or exact solution keys.
- Full answers remain available only to config, reports, and developer data-panel fields for tuning and QA.

# BattleSandbox Dev Chapter Playable Report
...
- Reuses `BattleSandboxRuntimeLoop01`, `BattleSandboxPlayableLoop01`, and `BattleSandboxPlayableFullRosterRegression01` evidence.
- Runtime selector binding uses the existing `DevChapterDropdownSlot`; no RectTransform layout rewrite or scene binder run is required.
- No formal RunFlow, StageConfig, Boss table, Reward, SaveData, Drop, Chapter progress, or V0.2/V0.3 chapter surface is touched.
- Player-facing text keeps full answers, DropBias weights, and Boss six-key answers hidden.

## Requirement Summary

| Requirement | Result | Evidence |
| --- | --- | --- |
| 3-10 selectable | `PASS` | scenarios=2 |
| 4-10 selectable | `PASS` | scenarios=2 |
| Different enemy/Boss profile | `PASS` | distinctProfiles=4 |
| Different mechanic feedback | `PASS` | feedbackRows=4 |
| Different Build pressure | `PASS` | pressureRows=4 |
| Victory/failure | `PASS` | victoryRows=2; defeatRows=2 |
| Restart/switch | `PASS` | restart=True; switch=True |
| Runtime reuse | `PASS` | runtime=V0.4-BattleSandboxRuntimeLoop01 |
| PlayableLoop/FullRoster reuse | `PASS` | playable=True; fullRoster=True |
| Player masking | `PASS` | playerLeaks=0 |
| Formal isolation | `PASS` | formalLeaks=0; flags=0 |
| UI layout preservation | `PASS` | runtimeLayoutWrites=0; selectorLayoutWrites=0; binder=False |

## Rows

| Row | Chapter | Target | Boss Profile | Result | Mechanic | Pressure | Pass |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `devChapterPlayable.dev_balance_3_10_guard_wall` | `3-10` | 叩阵铜将 | `dev_boss_burst_huzhen` | `Defeat` | `PASS` | `PASS` | `PASS` |
| `devChapterPlayable.dev_balance_3_10_cleanse_corner` | `3-10` | 秽签梦母 | `dev_boss_debuff_jinge` | `Victory` | `PASS` | `PASS` | `PASS` |
| `devChapterPlayable.dev_balance_4_10_furnace_core` | `4-10` | 偷灵炉心 | `dev_boss_energy_juneng` | `Victory` | `PASS` | `PASS` | `PASS` |
| `devChapterPlayable.dev_balance_4_10_thunder_fire_cross` | `4-10` | 黑炉复合阵眼 | `dev_boss_hybrid_combo` | `Defeat` | `PASS` | `PASS` | `PASS` |

## User Handtest Checklist

1. Open `Scene_TalismanBag_V04_BattleSandboxPreview` directly and press Play.
2. Click the dev chapter selector and confirm it switches between 3-10 and 4-10 without moving the UI layout.
3. Start sandbox battle on 3-10 and confirm the enemy/Boss name, mechanic feedback, and Build pressure text match that level.
4. Switch to 4-10 and confirm the enemy/Boss name, mechanic feedback, and Build pressure text change.
5. Let one run reach victory or failure, then click restart and confirm HP, shield, mana, log, cast bar, and result state reset for the same Build.
6. Switch level after a result and confirm the next run starts with the selected level.
7. Confirm no full answer, DropBias weight, or Boss six-key answer appears on the player side.
8. Confirm no SaveData, Reward, Drop, Chapter progress, formal RunFlow, formal StageConfig, or formal Boss table side effect occurs.

...
| White-key transparent material helper | PASS | Added BuildSandbox-local `UI_WhiteKeyTransparent` material/shader for UI Image sequence frames with white video backgrounds. It keys near-white pixels at render time with adjustable tolerance/softness, leaves source frame assets unchanged, and does not affect placement/session logic. |
...
| Prepare/continue sprite sequence helper | PASS | `BuildSandboxSpriteSequencePlayer` is optional and triggered only by opening prepare mode or continuing battle through the existing sandbox buttons; it does not affect placement session, receivers, save data, or formal flow. |
| Prepare/continue sequence autobind | PASS | If the optional controller field is empty, `BuildGridInteractionPreviewController` resolves an existing `BuildSandboxSpriteSequencePlayer` at runtime, preferring children, then the scene object named `guajian`, then the first player with frames. This is BuildSandbox-only and does not modify scene layout or formal flow. |
| Editable item-info panel | PASS | `BuildSandboxItemInfoPanel_Runtime` can be created/bound as a normal Edit Mode hierarchy object under `PopupLayer`; runtime reuses the scene object, existing scene-backed panels are not marked `DontSave`, and default layout writes are limited to newly created panel pieces. |
